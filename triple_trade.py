...
def check_trend(fast, slow, i):
    return "up" if fast[i] > slow[i] else "down"
...
def GO_TRADE(fast_tp, slow_tp):
    ...
    data['EURTRY_fast'] = data['EURTRY_Close'].rolling(fast_tp).mean()
    data['EURTRY_slow'] = data['EURTRY_Close'].rolling(slow_tp).mean()
    data['USDTRY_fast'] = data['USDTRY_Close'].rolling(fast_tp).mean()
    data['USDTRY_slow'] = data['USDTRY_Close'].rolling(slow_tp).mean()
    ...
    for i in range(1, len(data)):
        if len(trades) == 0:
            if check_buy(data['EURTRY_fast'],data['EURTRY_slow'],i) or \
                check_sell(data['USDTRY_fast'],data['USDTRY_slow'],i):
                trades.append(start_trade(data['EURUSD_Close'], QUANTITY, i))
            elif check_buy(data['USDTRY_fast'],data['USDTRY_slow'],i) or \
                check_sell(data['EURTRY_fast'],data['EURTRY_slow'],i):
                trades.append(start_trade(data['EURUSD_Close'], -QUANTITY, i))
        else:
            is_holding = trades[-1]['end_price'] == None
            trade_type = 'sell' if trades[-1]['quantity'] < 0 else 'buy'
            if is_holding:
                if trade_type == 'sell' and (check_buy(data['EURTRY_fast'],data['EURTRY_slow'],i) or \
                                            check_sell(data['USDTRY_fast'],data['USDTRY_slow'],i)):
                    trades[-1] = close_trade(trades[-1],data['EURUSD_Close'],i)
                    trades.append(start_trade(data['EURUSD_Close'], QUANTITY, i))
                elif trade_type == 'buy' and (check_buy(data['USDTRY_fast'],data['USDTRY_slow'],i) or \
                                            check_sell(data['EURTRY_fast'],data['EURTRY_slow'],i)):
                    trades[-1] = close_trade(trades[-1],data['EURUSD_Close'],i)
                    trades.append(start_trade(data['EURUSD_Close'], -QUANTITY, i))
            else:
...
for trade in trades:
    for i in range(trade['start_index']+1, trade['end_index']):
        trade_type = 'sell' if trades[-1]['quantity'] < 0 else 'buy'
        if trade_type == 'sell':
            dd = (trade['start_price'] - data['EURUSD_Close'][i])/trade['start_price']
        else:
            dd = (data['EURUSD_Close'][i] - trade['start_price'])/trade['start_price']
